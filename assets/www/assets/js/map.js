...
function demarrageApps(){
	...
	if(check_network()){
		...
		jAlert('Aucune connexion internet est disponible.', 'Ooupps',function() {
...
//		############################
		// Bossule
		// ############################
		L.Control.BoussoleNi = L.Control.extend({
			includes: L.Mixin.Events,
			options: {
				position: 'topleft',
				title: 'Boussole',
			...
			onAdd: function(map) {
				...
				L.DomEvent
				.addListener(map, 'locationfound', L.DomEvent.stopPropagation)
				.addListener(map, 'click', L.DomEvent.preventDefault)
				.addListener(map, 'drag', this.startWatch , this)
				.addListener(document.getElementById('search-address'), 'submit', this.startWatch , this);
		...
		L.control.boussole = function(map) {
			var bsl=new L.Control.BoussoleNi(map);
		...
		var Ortho44 = {
				...
				_geocode: function (event) {
					...
					if(check_network()){
						...
						jAlert('Vérifiez votre connexion internet SVP.', 'Ooupps',function() {
		...
		var ortho1850= L.tileLayer('http://{s}.tiles.cg44.makina-corpus.net/ortho-1850/{z}/{x}/{y}.jpg', {
			maxZoom: 16,
			tms: true,
			subdomains: 'abcdefgh'
		}),
...
//		}).addTo(map);
		...
		map.on('locationerror', function() {
			jAlert('Veuillez activer votre service de géolocalisation SVP.', 'Ooupps',function() {
		...
		Ortho44.bindGeocode(
				...
				function (results) {
					...
					if(results.hits.total > 0) {
						...
						Ortho44.removeClass(document.getElementById('search-address'), "search-success");
						document.getElementById("search-input").value ="Aucun résultat trouvé.";
...
//						Ortho44.setClass(choices_box, "show-choices");
		...
		function onLocationFound(e) {
			...
			if(max_bounds_forlocation.contains(e.latlng)){
				...
				var markerPosition=	L.marker([latitude, longitude]);
				markerPosition.bindPopup("<b>Votre position</b>").openPopup();
				...
				jAlert('Pas de vue aérienne pour votre position', 'Ooupps',function() {
